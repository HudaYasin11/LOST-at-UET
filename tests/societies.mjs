...
assert.equal(societies.length, 43);
assert.equal(new Set(societies.map(s => s.id)).size, 43);
...
assert.equal(findSocieties().length, 43);
...
assert.equal(societies.filter(s => s.logo).length, 31);
